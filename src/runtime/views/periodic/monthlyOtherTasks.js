...
const monthlyOtherT = (key, params = {}) => {
  try {
...
if (!yearFromPath || !monthMatch || monthMatch[1] !== yearFromPath) {
  ctx.paragraph(monthlyOtherT("runtime.periodic.otherTasks.monthNameInvalid"));
  return;
}
const monthStr = `${monthMatch[1]}-${monthMatch[2]}`;

const start = window.moment(monthStr, "YYYY-MM").startOf("month");
const end = window.moment(monthStr, "YYYY-MM").endOf("month");
...
const markTaskDone = async (task) => {
  try {
    ...
    try {
      bridge.refresh?.requestRefresh?.("stats", "monthly-other-task-done");
    } catch (_) {}
    return true;
  } catch {
    return false;
...
const root = ctx.el("div", "", { cls: "noria-periodic-task-list monthly-other-tasks" });
if (tasks.length === 0) {
  root.createEl("div", { text: monthlyOtherT("runtime.periodic.otherTasks.monthEmpty") }).style.cssText = "color:var(--text-muted);";
  return;
}
