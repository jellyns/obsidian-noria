/**
 * 运行视图内 Obsidian setIcon 常不可用；提供 Lucide 风格 SVG 后备 + window.require 探测。
 * 挂载：globalThis.dashboardCore.utils.applyLucideIcon(el, iconId)
 */
(() => {
  ...
  const SVG = {
    ...
    "file-plus-2":
      '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 22h14a2 2 0 0 0 2-2V7l-5-5H6a2 2 0 0 0-2 2v4"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M3 15h6"/><path d="M6 12v6"/></svg>',
    "square-pen":
    ...
    eye:
      '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2.06 12.35a1 1 0 0 1 0-.7C3.46 8.09 6.92 5.5 12 5.5s8.54 2.59 9.94 6.15a1 1 0 0 1 0 .7C20.54 15.91 17.08 18.5 12 18.5s-8.54-2.59-9.94-6.15"/><circle cx="12" cy="12" r="3"/></svg>',
    "eye-off":
      '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m2 2 20 20"/><path d="M10.6 10.6a2 2 0 0 0 2.8 2.8"/><path d="M9.88 4.24A10.5 10.5 0 0 1 12 4c5 0 8.5 3 10 8a12.2 12.2 0 0 1-2 3.5"/><path d="M6.6 6.6A12.4 12.4 0 0 0 2 12c1.5 5 5 8 10 8a10.4 10.4 0 0 0 4.1-.8"/></svg>',
    "folder-kanban":
  ...
  function trySetIcon(el, iconId) {
    try {
      const req = typeof require !== "undefined" ? require : globalThis.require;
      const obs = req && req("obsidian");
      if (obs && typeof obs.setIcon === "function") {
        el.replaceChildren();
        obs.setIcon(el, iconId);
        return !el.querySelector || !!el.querySelector("svg");
      }
    } catch (_) {}
    try {
      const wreq = typeof window !== "undefined" && window.require;
      const obs = wreq && wreq("obsidian");
      if (obs && typeof obs.setIcon === "function") {
        el.replaceChildren();
        obs.setIcon(el, iconId);
        return !el.querySelector || !!el.querySelector("svg");
  ...
  /** @param {HTMLElement} el @param {string} iconId Lucide id */
  function applyLucideIcon(el, iconId) {
    if (!el) return;
    const id = String(iconId || "").trim() || "list-checks";
    if (trySetIcon(el, id)) return;
    el.innerHTML = SVG[id] || SVG["list-checks"];
  }

  root.utils.applyLucideIcon = applyLucideIcon;
})();
