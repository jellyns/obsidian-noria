...
Noria 日历位于侧边栏，用于定位和创建日、周、月、季、年周期笔记，让周期计划、日常记录与复盘可以从日期直接进入对应 Markdown。
...
1. 在主页的项目区找到一个项目。首次初始化后，可以先使用其中的示例项目。
2. 在 **记下一步** 中输入一条具体行动并添加，任务会写入该项目的 Markdown 笔记。
3. 点击任务打开来源，将光标放在任务行，运行 **Noria：编辑/创建光标处任务**，设置排期或完成状态。
4. 打开任务看板或任务时间轴查看已排期任务，做完后标记完成；修改保留在原始笔记中，并同步到任务视图。
