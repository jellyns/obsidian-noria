...
test("inbox status migration rewrites only inboxRoot markdown files and is idempotent", async () => {
  const plugin = makePlugin({
    files: {
      "Inbox/a.md": "---\ninbox-status: 待决\n---\n\n# A",
      "Inbox/b.md": "---\ninbox-status: \"加工中\"\n---\n\n# B",
  ...
  plugin.settings = plugin.normalizeSettings({
    ...
    inboxWorkflow: {
      statuses: [
        { id: "capture", label: "Capture", aliases: ["待决"] },
        { id: "working", label: "Working", aliases: ["加工中"] },
        { id: "done", label: "Done", aliases: ["已关闭"], terminal: true }
      ],
      defaultStatusId: "capture"
    }
  ...
  const first = await plugin.migrateInboxStatusFrontmatter({ silent: true });
  assert.equal(first.migrated, 2);
  assert.equal(first.skipped, 1);
  assert.match(plugin.__files.get("Inbox/a.md").text, /inbox-status: capture/);
  assert.match(plugin.__files.get("Inbox/b.md").text, /inbox-status: working/);
  assert.match(plugin.__files.get("Inbox/c.md").text, /inbox-status: waiting/);
  assert.match(plugin.__files.get("Archive/d.md").text, /inbox-status: 待决/);

  const second = await plugin.migrateInboxStatusFrontmatter({ silent: true });
  assert.equal(second.migrated, 0);
});

test("inbox status migration preserves concurrent body edits through vault.process", async () => {
  const filePath = "Inbox/a.md";
  const plugin = makePlugin({
    files: {
      [filePath]: "---\ninbox-status: 待决\n---\n\n# A\n"
    }
  ...
  const result = await plugin.migrateInboxStatusFrontmatter({ silent: true });

  assert.equal(result.migrated, 1);
  assert.equal(processCalls, 1);
  const written = plugin.__files.get(filePath)?.text || "";
  assert.match(written, /inbox-status: capture/);
  assert.match(written, /Concurrent body edit\./);
});
